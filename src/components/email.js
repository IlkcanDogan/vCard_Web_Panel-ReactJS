...
function Email({index, types, value, typeId, onChange, onRemove, error}) {
    return (
        <div className="form-group row thin">
            <div className="col-12 col-sm-3 col-form-wrapper">
                <div className="d-block d-sm-none">
                    <label className="col-form-label">E-posta Adresi </label>
            ...
            <div className="col-12 col-sm-9">
                <div className="d-flex">
                    <div className="row no-gutters flex-grow-1">
                        <div className="col-12 col-sm-3">
                            <select className={`form-control ${error.id === 'type' && error.part === 'email' && error.index === index ? 'is-invalid' : null}`} onChange={(e) => onChange({id: e.target.value, email: value})}>
                        ...
                        <div className="col-12 col-sm-9">
                            <input 
                                className={`form-control ${error.id === 'email' && error.index === index ? 'is-invalid' : null}`}
                                autoComplete="email" 
                                type="text" 
                                placeholder="E-posta" 
                                value={value} 
                                onChange={(e) => onChange({id: typeId, email: e.target.value})}
                            />
                        </div>
                    </div>
                    <div className="d-none d-sm-block">
                ...
                {error.part === 'email' && error.index === index ? (
