...
function Phone({ index, types, value, typeId, onChange, onRemove, error }) {
    return (
        <div className="form-group row thin">
            ...
            <div className="col-12 col-sm-9">
                <div className="d-flex">
                    <div className="row no-gutters flex-grow-1">
                        <div className="col-12 col-sm-3">
                            <select className={`form-control ${error.id === 'type' && error.part === 'phone' && error.index === index ? 'is-invalid' : null}`} onChange={(e) => onChange({ id: e.target.value, number: value })}>
                        ...
                        <div className="col-12 col-sm-9">
                            <input
                                className={`form-control ${error.id === 'number' && error.index === index ? 'is-invalid' : null}`}
                                autoComplete="tel"
                                type="tel"
                                placeholder="Numara"
                                value={value}
                                onChange={(e) => onChange({ id: typeId, number: e.target.value })}
                            />
                        </div>
                    </div>
                    <div className="d-none d-sm-block">
                ...
                {error.part === 'phone' && error.index === index ? (
