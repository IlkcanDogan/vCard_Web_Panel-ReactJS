...
function Edit() {
    ...
    const [profile, setProfile] = useState({cardName: '', email: '', password: '', firstname: '', lastname: '', birthday: '', orgName: '', position: '', note: '', createdDate: '', savedPhoto: null, photo: ''});
    const [phones, setPhones] = useState([]); 
    const [emails, setEmails] = useState([]);
    const [websites, setWebsites] = useState([]);
    const [addresses, setAddresses] = useState([]);

    const [deleted, setDeleted] = useState({phones: [], emails: [], websites: [], addresses: []});
            ...
            axios.get(API_URL + '/card.php?profile_code=' + profileCode).then((resp) => {
                if (resp.data.status === 'success') {
                    ...
                    setProfile({
                        ...
                        savedPhoto: DOMAIN + '/photo/' + profileCode + '.blob'
                    });
                    //#endregion
    ...
    const [updateReqWait, setUpdateReqWait] = useState(false);
    const handleUpdate = () => {
        setUpdateReqWait(true);
        //#region Photo
        if(!profile.savedPhoto && !profile.photo) {
            axios.post(API_URL + '/photo_remove.php', {
                profile_code: profileCode,
                email: profile.email,
                password: profile.password
            }).then((resp) =>{
                if(resp.data.status === 'success') {
                    //history.push('/p/' + profileCode);
                }
                else {
            ...
            if(profile.photo) {
                ...
                fetch(profile.photo).then((file) => {
                    file.blob().then((theBlob) => {
                        ...
                        axios.post(API_URL + '/photo_update.php', formData, { headers: { 'Content-Type': 'multipart/form-data' }}).then((resp) => {
                            if(resp.data.status === 'success') {
                                //history.push('/p/' + profileCode);
                            }
                            else {
        ...
        //#region Card
        axios.post(API_URL + '/card_update.php', {
            profile_code: profileCode,
            profile: profile,
            phones: phones,
            emails: emails,
                    ...
                    <div>
                        ...
                        <div className="card-create-process-container">
                            <div className="card-create-process">
                                <p>KartDijital çevrimiçi bir kartvizit oluşturabilir ve bunu kişilerinizle paylaşabilirsiniz.</p>
                                        ...
                                        <div className="col-12 col-sm-9">
                                            <div className="photo-dropzone">
                                                <div className="avatar-container">
                                                    ...
                                                    <img src={profile.savedPhoto + '?random_number=' + new Date().getTime()} style={{ borderRadius: "50%", height: 180, width: 180 }}
                                                        onError={() => setProfile({ ...profile, savedPhoto: null })}
                                                    />
                                                    <div className="avatar-upload-button mt-2">
                                                        <button title="Fotoğrafı Sil" type="button" className="ml-1 btn btn-danger" onClick={() => setProfile({ ...profile, savedPhoto: null })}>
                                    ...
                                    return (
                                        <Phone
                                            ...
                                            onChange={(data) => {
                                                ...
                                                tmpPhones[index] = { ...phone, typeId: data.id, number: data.number }
                                                setPhones(tmpPhones);
                                                setError(defaultError);
                                            }}
                                            onRemove={() => {
                                                ...
                                                if(phone.id) setDeleted({...deleted, phones: [...deleted.phones, phone.id]});
                                            }}
                                            error={error}
                                            index={index}
                                        />
                                    ...
                                    return (
                                        <Email
                                            ...
                                            onChange={(data) => {
                                                ...
                                                tmpEmails[index] = { ...email, typeId: data.id, email: data.email }
                                                setEmails(tmpEmails);
                                                setError(defaultError);
                                            }}
                                            onRemove={() => {
                                                ...
                                                if(email.id) setDeleted({...deleted, emails: [...deleted.emails, email.id]});
                                            }}
                                            error={error}
                                            index={index}
                                        />
                                    ...
                                    return (
                                        <Website
                                            ...
                                            onChange={(data) => {
                                                ...
                                                tmpWebsites[index] = { ...website, typeId: data.id, url: data.url }
                                                setWebsites(tmpWebsites);
                                                setError(defaultError);
                                            }}
                                            onRemove={() => {
                                                ...
                                                if(website.id) setDeleted({...deleted, websites: [...deleted.websites, website.id]});
                                            }}
                                            error={error}
                                            index={index}
                                        />
                                    ...
                                    return (
                                        <Address
                                            ...
                                            onRemove={() => {
                                                ...
                                                if(address.id) setDeleted({...deleted, addresses: [...deleted.addresses, address.id]});
                                            }}
                                            error={error}
                                            index={index}
                                        />
                                ...
                                <div className="form-group row">
                                    ...
                                    <div className="col-12 col-sm-9">
                                        <textarea
                                            className="form-control"
                                            rows="5"
                                            onChange={(e) => setProfile({ ...profile, note: e.target.value.replace(/'/g, '"') })}
                                ...
                                <div className="form-group row">
                                    <div className="col-12 col-sm-9 offset-sm-3">
                                        ...
                                        <div className="btn-toolbar">
                                            <button type="button" className="btn btn-primary" onClick={handleControl}>{updateReqWait ? 'Lütfen bekleyin...' : 'Güncelle'}</button>
                                            <Link to={'/p/' + profileCode} class="btn btn-default">İptal</Link>
                                        </div>
                                    </div>
                                </div>
                                <div className="form-group row">
